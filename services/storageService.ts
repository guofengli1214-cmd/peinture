...
export const getS3Config = (): S3Config => {
  return useConfigStore.getState().s3Config || DEFAULT_S3_CONFIG;
};

export const getWebDAVConfig = (): WebDAVConfig => {
  return useConfigStore.getState().webdavConfig || DEFAULT_WEBDAV_CONFIG;
};

export const getStorageType = (): StorageType => {
  return useConfigStore.getState().storageType || "opfs";
};
...
export const saveS3Config = (config: S3Config) =>
  useConfigStore.getState().setS3Config(config);
export const saveWebDAVConfig = (config: WebDAVConfig) =>
  useConfigStore.getState().setWebDAVConfig(config);
export const saveStorageType = (type: StorageType) =>
  useConfigStore.getState().setStorageType(type);
...
// --- Unified AWS Signature V4 Signing ---
...
/**
 * Computes an AWS Signature V4 for an S3 request.
 * Centralizes the signing logic used by upload, list, delete, test, and fetch operations.
 */
const signS3Request = async (
  ...
  return { authorizationHeader, isoDate, payloadHash, host };
};

/** Build S3 endpoint and host from config */
const getS3EndpointInfo = (config: S3Config) => {
  const region = config.region || "us-east-1";
  let endpoint = config.endpoint || `https://s3.${region}.amazonaws.com`;
  endpoint = endpoint.replace(/\/$/, "");
  const host = new URL(endpoint).host;
  const bucket = config.bucket || "";
  return { endpoint, host, bucket };
};
...
export const uploadToCloud = async (
  ...
  if (metadata) {
    try {
      ...
      const metadataFileName = id
        ? `${id}.metadata.json`
        : `${finalFileName}.metadata.json`;

      const metadataContent = { ...metadata };
      if (!metadataContent.id) metadataContent.id = id;

      const jsonBlob = new Blob([JSON.stringify(metadataContent, null, 2)], {
...
const fetchS3Signed = async (
  ...
  const signed = await signS3Request(config, method, path, query, payloadHash);
...
export const renameCloudFile = async (
  oldKeyOrUrl: string,
  newKeyOrUrl: string,
): Promise<void> => {
  const type = getStorageType();
  ...
  if (isManagedStorageType(type)) {
    ...
    const readUrl =
      oldKeyOrUrl.startsWith("opfs://") || oldKeyOrUrl.startsWith("/")
        ? oldKeyOrUrl
    ...
    const oldFileName = oldKeyOrUrl
      .replace("opfs://", "")
      .replace(`${OPFS_GALLERY_DIR}/`, "")
      .split("/")
      .pop();
...
const performS3Rename = async (
  ...
  const constructUrl = (key: string) =>
    bucket ? `${endpoint}/${bucket}/${key}` : `${endpoint}/${key}`;

  const oldUrl = oldKeyOrUrl.startsWith("http")
    ? oldKeyOrUrl
    : constructUrl(oldKeyOrUrl);

  const blob = await fetchCloudBlob(oldUrl);

  const newKey = newKeyOrUrl.split("/").pop() || newKeyOrUrl;

  await uploadToS3(blob, newKey, blob.type, config);

  const oldKey = oldKeyOrUrl.startsWith("http")
    ? oldKeyOrUrl.split("/").pop()!
    : oldKeyOrUrl;
  await deleteS3Object(config, oldKey);
};

const performWebDAVRename = async (
  config: WebDAVConfig,
  oldKeyOrUrl: string,
  newKeyOrUrl: string,
) => {
  const sourceUrl = oldKeyOrUrl.startsWith("http")
    ? oldKeyOrUrl
    : joinPath(config.url, config.directory, oldKeyOrUrl);
  const destUrl = newKeyOrUrl.startsWith("http")
    ? newKeyOrUrl
    : joinPath(config.url, config.directory, newKeyOrUrl);
...
// Init directories — cached to avoid redundant I/O on every write
...
export const renameTempFileFromOPFS = async (
  oldFileName: string,
  newFileName: string,
) => {
  try {
...
const listOPFSGalleryFiles = async () => {
  ...
  const galleryHandle = await root.getDirectoryHandle("gallery", {
    create: true,
  });
...
const uploadToS3 = async (
  ...
  const { endpoint, bucket } = getS3EndpointInfo(config);
  ...
  const signed = await signS3Request(config, "PUT", uriPath, "", payloadHash);
...
export const listS3Files = async (config: S3Config): Promise<CloudFile[]> => {
  ...
  const payloadHash = await sha256("");
  const signed = await signS3Request(config, "GET", canonicalUri, queryString, payloadHash);
  ...
  try {
    ...
    for (let i = 0; i < contents.length; i++) {
      ...
      const size = parseInt(
        contents[i].getElementsByTagName("Size")[0].textContent || "0",
        10,
      );
...
export const deleteS3Object = async (
  ...
  const payloadHash = await sha256("");
  const signed = await signS3Request(config, "DELETE", uriPath, "", payloadHash);
...
const listWebDAVFiles = async (config: WebDAVConfig): Promise<CloudFile[]> => {
  ...
  try {
    ...
    for (let i = 0; i < responses.length; i++) {
      ...
      if (decodedPath.replace(/\/$/, "") === decodedBasePath.replace(/\/$/, ""))
        continue;
...
export const testWebDAVConnection = async (
  ...
  try {
    ...
    if (!rootResponse.ok) {
      return {
        success: false,
        message: `Connection failed: ${rootResponse.status}`,
      };
    }
    return { success: true, message: "Connection successful" };
  } catch (e: any) {
