...
export const Select: React.FC<SelectProps> = ({
  ...
  const selectedOption =
    flattenedOptions.find((opt) => opt.value === value) || flattenedOptions[0];
  ...
  useEffect(() => {
    if (isOpen && containerRef.current) {
      ...
      if (spaceBelow < estimatedHeight && spaceAbove > estimatedHeight) {
        ...
        calculatedMaxHeight = Math.max(120, Math.min(300, spaceAbove - 20));
      } else {
        setMenuPosition("bottom");
        // Recalculate max height for bottom position
        calculatedMaxHeight = Math.max(120, Math.min(300, spaceBelow - 20));
      }
  ...
  return (
    <div
      className={`group relative ${horizontal ? "flex items-center gap-4" : ""}`}
      ref={containerRef}
    >
      <div
        className={
          horizontal
            ? "w-1/3 flex-shrink-0"
            : `flex items-center justify-between ${dense ? "pb-1.5" : "pb-3"}`
        }
      >
        <p
